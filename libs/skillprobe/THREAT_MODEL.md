# Threat Model: skillprobe

> Scope: `libs/skillprobe/src/skillprobe/` — the review library, CLI and UI.

skillprobe exists to judge untrusted agent skills. That makes the tool itself a
place where untrusted input is handled, so its own trust boundaries matter as
much as the findings it reports.

## What skillprobe defends against
...
across eight scanners). skillprobe judges the skill by the operations it
...
- Runtime protection of a deployed agent — skillprobe is a pre-publication gate,
